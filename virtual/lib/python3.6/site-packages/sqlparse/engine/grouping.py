...
def group_typecasts(tlist):
    ...
    def assets(tlist, pidx, tidx, nidx):
        return pidx, nidx

    valid_prev = valid_next = valid
    _group(tlist, sql.Identifier, match, valid_prev, valid_next, assets)
...
def group_assignment(tlist):
    ...
    valid_prev = valid_next = valid
    _group(tlist, sql.Assignment, match, valid_prev, valid_next, assets)
...
def group_comparison(tlist):
    ...
    _group(tlist, sql.Comparison, match,
           valid_prev, valid_next, assets, extend=False)
...
def group_arrays(tlist):
    ...
    _group(tlist, sql.Identifier, match,
           valid_prev, valid_next, assets, extend=True, recurse=False)
...
def group_identifier_list(tlist):
    ...
    _group(tlist, sql.IdentifierList, match,
           valid_prev, valid_next, assets, extend=True)
...
def _group(tlist, cls, match,
    ...
    for idx, token in enumerate(list(tlist)):
        ...
        if recurse and token.is_group and not isinstance(token, cls):
            _group(token, cls, match, valid_prev, valid_next, assets, extend)

        if match(token):
            nidx, next_ = tlist.token_next(tidx)
            if prev_ and valid_prev(prev_) and valid_next(next_):
                from_idx, to_idx = assets(tlist, pidx, tidx, nidx)
